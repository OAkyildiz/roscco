...
MyRideOSCC::MyRideOSCC()
{
   ...
   //FROM OBDII TO CONTROLLER
    ...
    obd2_throttle_pub = nh.advertise<std_msgs::Int16>("oscc/brake", 1);
    obd2_brake_pub = nh.advertise<std_msgs::Int16>("oscc/throttle", 1);
...
void MyRideOSCC::speedCmdCallback( const std_msgs::Float32::ConstPtr& input ){
    ...
    if (v_d == 0){
        ...
        throttle_msg.throttle_position = throttle_report + 0.05 *error;
    ...
    ROS_DEBUG("CMD - Brake: %f    Throttle: %f",throttle_msg.throttle_position, brake_msg.brake_position);
...
void MyRideOSCC::steeringCmdCallback( const std_msgs::Float32::ConstPtr& input ) 
{
    ...
    ROS_DEBUG("CMD - Steering: %f", output.steering_torque);
...
void MyRideOSCC::canFrameCallback( const roscco::CanFrame& input ) 
{
    ...
    int_msg.data = int(throttle_report);
    obd2_throttle_pub.publish(int_msg);

    int_msg.data = int(brake_report);
    obd2_brake_pub.publish(int_msg);

    int_msg.data = int(steering_angle_report);
    obd2_steer_angle_pub.publish(int_msg);
    ROS_DEBUG("[REP] Brake: %f  Throttle: %f Steering: %f ", throttle_report, brake_report, steering_angle_report);
